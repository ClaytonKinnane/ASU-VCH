...
if ((int) $user['must_change_password'] !== 1) { redirect('/admin/'); }
...
<head>
    ...
    <link rel="stylesheet" href="<?= e(theme_asset('css/account.css')) ?>">
</head>
<body>
...
<section class="auth-card password-change-card glass-tile">
    ...
    <form method="post" action="/account/change-password.php" novalidate>
        ...
        <div class="form-group"><label class="form-label" for="current_password">Текущий пароль</label><div class="password-wrapper"><input class="form-input" id="current_password" name="current_password" type="password" autocomplete="current-password" required><button class="password-toggle" type="button" data-password-toggle="current_password" aria-pressed="false" aria-label="Показать текущий пароль">Показать</button></div><?= required_password_field_error($errors, 'current_password') ?></div>
        <div class="form-group"><label class="form-label" for="new_password">Новый пароль</label><div class="password-wrapper"><input class="form-input" id="new_password" name="new_password" type="password" minlength="10" maxlength="128" autocomplete="new-password" required><button class="password-toggle" type="button" data-password-toggle="new_password" aria-pressed="false" aria-label="Показать новый пароль">Показать</button></div><?= required_password_field_error($errors, 'new_password') ?></div>
        <div class="form-group"><label class="form-label" for="new_password_confirmation">Повторите новый пароль</label><div class="password-wrapper"><input class="form-input" id="new_password_confirmation" name="new_password_confirmation" type="password" minlength="10" maxlength="128" autocomplete="new-password" required><button class="password-toggle" type="button" data-password-toggle="new_password_confirmation" aria-pressed="false" aria-label="Показать подтверждение нового пароля">Показать</button></div><?= required_password_field_error($errors, 'new_password_confirmation') ?></div>
