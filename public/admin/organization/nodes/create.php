...
organization_handle_action(function () use ($user): string {
    ...
    organizational_structure_service()->addNode($versionId, organization_positive_int($_POST['parent_node_id'] ?? null), organization_positive_int($_POST['type_id'] ?? null), organization_post_nullable_string('internal_code'), organization_post_string('name'), organization_post_nullable_string('short_name'), organization_post_nullable_string('note'), $revision, (int) $user['id']);
