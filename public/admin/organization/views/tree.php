    <section id="tree" class="organization-panel glass-tile">
        ...
        <div class="organization-tree" data-organization-tree>
            <?php foreach ($flatTree as $entry): $node = $entry['node']; $depth = (int) $entry['depth']; $nodeTypeOptions = $node['parent_node_id'] === null ? $rootTypes : $types; ?>
            <article class="organization-tree-row" style="--tree-depth: <?= $depth ?>" data-tree-node data-parent-id="<?= $node['parent_node_id'] !== null ? (int) $node['parent_node_id'] : 0 ?>" data-node-id="<?= (int) $node['id'] ?>" data-search-text="<?= e(mb_strtolower((string) $node['name'] . ' ' . (string) ($node['internal_code'] ?? '') . ' ' . (string) $node['type_name'])) ?>">
