...
function Assert-PathSet {
    ...
    if ($Difference.Count -ne 0) {
        Write-Host 'Expected paths:' -ForegroundColor DarkYellow
        $ExpectedSorted | ForEach-Object { Write-Host "  $_" }
        Write-Host 'Actual paths:' -ForegroundColor DarkYellow
        $ActualSorted | ForEach-Object { Write-Host "  $_" }
        throw 'Implementation path set does not match the approved scope.'
    }
}
...
function Assert-SourceDeployParity {
    ...
    foreach ($RelativePath in $RelativePaths) {
        ...
        if (-not (Test-Path -LiteralPath $Target -PathType Leaf)) {
            throw "Deploy file was not found: $Target"
        }
...
$ExpectedPaths = @(
    ...
    'docs/testing/PUBLIC-MILITARY-OCCUPATIONAL-SPECIALTIES-V1-MANUAL-DESKTOP-ACCEPTANCE-2026-08-01.md',
...
if (@(git status --porcelain).Count -ne 0) {
    throw 'Working tree must be clean before Testing.'
}
...
if ($Branch -ne $ExpectedBranch) {
    throw "Expected branch $ExpectedBranch, actual branch: $Branch"
}
if ($OriginMain -ne $ExpectedBaseSha) {
    throw "origin/main changed after baseline approval: $OriginMain"
}
if ($Head -ne $OriginFeature) {
    throw 'Local HEAD does not match the remote feature branch.'
}
if ($Divergence -notmatch '^0\s+0$') {
    throw "Local feature branch diverges from origin: $Divergence"
}
if ($MergeBase -ne $ExpectedBaseSha) {
    throw "Merge base does not match the approved baseline: $MergeBase"
}
...
if ($LASTEXITCODE -ne 0 -or $DiffCheckOutput.Count -ne 0) {
    throw 'git diff --check found errors.'
}
...
if (-not (Test-Path -LiteralPath $DeployConfigPath -PathType Leaf)) {
    throw "Deploy config was not found: $DeployConfigPath"
}
...
Invoke-External 'powershell.exe' @(
    ...
    'Bypass',
    '-File',
    (Join-Path $RepositoryRoot 'tools\Backup-Database.ps1'),
    '-DeployRoot',
    $DeployRoot
)
...
Invoke-External 'powershell.exe' @(
    ...
    'Bypass',
    '-File',
    (Join-Path $RepositoryRoot 'deploy\Deploy-Local.ps1')
)
...
if ($ConfigHashAfterDeploy -ne $ConfigHashBefore) {
    throw 'Deploy changed config/local.php.'
}
...
Invoke-External 'powershell.exe' @(
    ...
    'Bypass',
    '-File',
    (Join-Path $DeployRoot 'tools\Test-PhpSyntax.ps1'),
    '-PhpExecutable',
    'php'
)
...
$SmokeArguments = @(
    ...
    'Bypass',
    '-File',
    (Join-Path $DeployRoot 'tools\Test-LocalSmoke.ps1')
)
...
if ($ConfigHashFinal -ne $ConfigHashBefore) {
    throw 'Final config/local.php SHA-256 changed.'
}
...
if ($FinalHead -ne $Head) {
    throw 'HEAD changed during Testing.'
}
if ($FinalDivergence -notmatch '^0\s+0$') {
    throw "Final divergence from origin: $FinalDivergence"
}
...
Write-Host 'MANUAL_DESKTOP_ACCEPTANCE_STATUS=TARGETED_RECHECK_REQUIRED'
Write-Host 'MOBILE_TESTING_STATUS=OUT_OF_SCOPE_NOT_RUN'
Write-Host 'PR_STATUS=OPEN_20_NOT_MERGED'
exit 0