...
function Test-CodexRequestedMode {
    param(
    ...
    switch ($RequestedMode) {
        'ChatGPT' {
            return (@('CHATGPT', 'UNKNOWN') -contains ([string]$State.Mode))
        }
        'ApiKey' {
            return (@('API_KEY', 'UNKNOWN') -contains ([string]$State.Mode))
        }
        default {
            return $true
        }
...
try {
    ...
    if ($CodexAuthMode -eq 'Skip') {
        ...
        $requestedModeReady = Test-CodexRequestedMode `
            -RequestedMode $selected `
            -State $codexState

        $authenticationRequired = (
            -not $codexState.Ready -or
            -not $requestedModeReady
        )

        if ($authenticationRequired -and $Mode -ne 'Doctor') {
            if ($selected -eq 'Auto') {
        ...
        if ($codexState.Ready -and -not $requestedModeReady) {
            $script:Capability.CODEX_AUTH_READY = 'NO'
            $script:Capability.CODEX_CHATGPT_AUTH_READY = 'NO'
            $script:Capability.CODEX_API_KEY_AUTH_READY = 'NO'
            $script:Pending.Add(
                "Codex authentication mode mismatch. Requested=$selected Actual=$($codexState.Mode)."
            )
            Write-Log -Level WARN -Message (
                "Codex authentication mode mismatch. Requested=$selected Actual=$($codexState.Mode)."
            )
        }
        elseif ($codexState.Ready) {
            if (
                $selected -in @('ChatGPT', 'ApiKey') -and
                $codexState.Mode -eq 'UNKNOWN'
            ) {
                Write-Log -Level WARN -Message (
                    "Codex authentication ready, but CLI mode wording is unknown. Requested=$selected."
                )
            }
            else {
                Write-Log -Level PASS -Message "Codex authentication ready. Mode=$($codexState.Mode)"
            }
        }
        else {
            Write-Log -Level WARN -Message 'Codex authentication pending.'
        }
    }
